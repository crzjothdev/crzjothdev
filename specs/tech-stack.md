...
Current state as of May 2026. Branch `phase-1/foundation-migration` completes the migration to App Router + TypeScript + Tailwind CSS.
...
| Item | Detail |
|---|---|
| **Next.js** | v15 — App Router (active on `phase-1/foundation-migration`) |
| **React** | v18.3 |
...
| **JavaScript** | Removed — no `.js` files remain in `components/` or `app/` after Phase 1 |
...
| Item | Detail |
|---|---|
| **Dev container name** | `alphawolf` (defined in `compose.yaml`) / `alpha-wolf` (legacy `docker-compose.yaml`, currently running) |
| **Dev Dockerfile** | `Dockerfile.dev` — base image `node:23-alpine`, runs `npm run dev` |
| **Compose file** | `compose.yaml` (current); legacy `docker-compose.yaml` also present |
...
| **Run a command** | `docker exec <container_name> <command>` — e.g. `docker exec alpha-wolf npm run build` |
...
| **Docker Compose** | `compose.yaml` for local dev; `docker-compose.yaml` (legacy) still present |
...
| **Run tests** | `docker exec <container_name> npm test` |
